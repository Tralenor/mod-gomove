...
-- MAIN FRAME (original size 170,455)
local MainFrame = GOMove:CreateFrame("GOMove_UI", 170, 485)
GOMove.MainFrame = MainFrame
MainFrame:Position("LEFT", UIParent, "LEFT", 0, 85)

-- TEST Button 								display text, width, height, pos-x (0 gleich in der mitte?) , pos-y from top
-- local TEST = GOMove:CreateButton(MainFrame, "Test", 100, 30, 0, -25)
-- setOnKlick Event action
...
local RADIUS = GOMove:CreateInput(MainFrame, "RADIUS", 40, 25, -55, -325, 4, 3)
...
--add Size +/- buttons               
--                                     4 = input length, 30 = default / start value
local SIZEMOD = GOMove:CreateInput(MainFrame, "SIZEMOD", 50, 25, 0, -445, 4, 1)
local SIZEPLUS = GOMove:CreateButton(MainFrame, "Size +", 50, 25, 55, -445)
function SIZEPLUS:OnClick()
    GOMove:Move("SIZEPLUS", SIZEMOD:GetNumber())
end

local SIZEMINUS = GOMove:CreateButton(MainFrame, "Size -", 50, 25, -55, -445)
function SIZEMINUS:OnClick()
    GOMove:Move("SIZEMINUS", SIZEMOD:GetNumber())
end
